...
* `paths` — every entry is a repository path: relative, no `..`, `.` or empty
  component, no backslash, no drive letter, nothing under `.git`, no symlink
  entry (a kit is whole files), and it lands inside `--repo` once the repo's own
...
* `verdicts` — every `verdict` regex on a `builder.done` rule matches (by
  `re.search`, as the engine does) at least one literal, placeholders such as
  `<unit>` left as text, and every literal matches some such rule. A rule that
  matches no literal of the brief passes only when it matches the one reply
  this command itself asks for, the apply prompt's `VERDICT: kit applied <sha>`.
  Verdict rules on other events (the review's `aux.done`) are counted and named
  on the line, not matched against the brief: the brief is the builder's
  vocabulary, not the reviewer's (H-021).
...
* `protocol` — every file a `send` rule's prompt names (a `.md` or `.toml` path
  without a `{placeholder}`) is in the kit or the repo.
"""
...
#: A file a prompt names: a relative `.md`/`.toml` path, not touching a `{placeholder}`.
_NAMED_FILE_RE = re.compile(r"(?<![\w{}./~-])([\w-][\w./-]*\.(?:md|toml))(?![\w{}/-])")
...
def _check_verdicts(book: Playbook | None, literals: list[str] | None) -> Check:
    if book is None:
    ...
    rules = [rule for rule in book.rules if rule.verdict is not None and rule.on == BUILDER_DONE]
    others = [rule for rule in book.rules if rule.verdict is not None and rule.on != BUILDER_DONE]
    problems: list[str] = []
    via_apply: list[str] = []
    for rule in rules:
    ...
    if others:
        events = sorted({rule.on for rule in others})
        notes.append(
            f"{len(others)} verdict rule(s) on {_and(events)} not matched against the brief "
            "(the brief fixes the builder's replies, not the review's; H-021)"
        )
    if problems:
    ...
    return Check("verdicts", True, "; ".join([head, *notes]))
...
def _check_protocol(book: Playbook | None, kit: _Kit, repo: Path) -> Check:
    ...
    for name, indexes in sorted(named.items()):
        ...
        if name in kit.files:
            found.append(f"{name} (kit)")
        elif _path_problem(name) is None and (repo / name).is_file():
            found.append(f"{name} (repo)")
        else:
            missing.append(f"{rules} names {name}, which is in neither the kit nor the repo")
    if missing:
